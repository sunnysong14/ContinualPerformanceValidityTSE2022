...
class OzaBagging_OOB(BaseSKMObject, ClassifierMixin, MetaEstimatorMixin):
    """ 
    ...
    def partial_fit(self, X, y, rho0, rho1, classes=None, sample_weight=None):
        """ 
        NOTE
        ...
        for j in range(r):
            for i in range(self.actual_n_estimators):
                ...
                if y[0] == 1 and rho0 > rho1:
                    our_lambda = rho0 / rho1
                elif y[0] == 0 and rho0 < rho1:
                    our_lambda = rho1 / rho0
                else:
                    our_lambda = 1

                k = self._random_state.poisson(our_lambda)
                if k > 0:
                    for b in range(k):
